...
// No imports needed — uses Web standard APIs only

const OG_IMAGE = "https://www.opstools.ai/og-image.png";

const PAGE_META = {
  "/": { title: "OpsTools — Free Business Document Generator for India", description: "Free online document generators for Indian small businesses. Fuel bills, rent receipts, GST invoices, salary slips — no login, no cost, instant PDF." },
  ...
  "/documents/rent-receipt": { title: "Free Rent Receipt Generator India — HRA Compliant | OpsTools", description: "Generate HRA-compliant rent receipts with landlord PAN, stamp and signature. Free, no login, instant PDF." },
  "/documents/ld-bill": { title: "Free L&D Tax Invoice Generator — Training & Courses | OpsTools", description: "Generate GST tax invoices for learning & development expenses. HSN code 998433, CGST/SGST/IGST support, instant PDF." },
  ...
  "/documents/service-invoice": { title: "Free Service Invoice Generator India — GST Service Bills | OpsTools", description: "Generate service invoices with GST and payment terms for contractors and agencies. Free, instant PDF." },
  ...
  "/documents/e-invoice": { title: "Free E-Invoice Generator India — GST E-Invoice with IRN | OpsTools", description: "Generate GST e-invoices with IRN, acknowledgement number and line items. Free, no login, instant PDF." },
  ...
  "/business/roi-calculator": { title: "Free ROI Calculator India — Return on Investment Tool | OpsTools", description: "Calculate ROI, compare investments, run what-if scenarios. FD, RD and savings account comparisons. Free." },
  "/business/gst-calculator": { title: "Free GST Calculator India — Add or Remove GST with CGST/SGST | OpsTools", description: "Calculate GST instantly. Add or remove GST. CGST, SGST and IGST breakdown for any rate. Free." },
  "/blogs": { title: "Blog — GST, Documents & Business Guides for India | OpsTools", description: "Practical guides for Indian small business owners — how to generate fuel bills, L&D invoices, GST documents and more." },
  "/blogs/how-to-generate-fuel-bill-online-india": { title: "How to Generate a Fuel Bill Online for Free in India (2026) | OpsTools", description: "Complete guide to generating petrol and diesel receipts online for reimbursement, expense tracking and fleet management." },
  "/blogs/how-to-generate-ld-bill-online-india": { title: "How to Generate an L&D Tax Invoice Online in India (2026) | OpsTools", description: "Complete guide to generating professional tax invoices for training, courses and learning & development expenses." },
  "/about": { title: "About OpsTools — Free Business Document Tools for India", description: "OpsTools is a free toolkit for Indian small business operators to generate professional documents without sign-ups or subscriptions." },
};

const CRAWLERS = ["linkedinbot","twitterbot","facebookexternalhit","whatsapp","telegrambot","slackbot","discordbot","googlebot","bingbot","applebot"];
...
export default async function middleware(request) {
  ...
  if (!isCrawler(ua)) return Response.next();

  const meta = PAGE_META[pathname];
  if (!meta) return Response.next();
  ...
  return new Response(injected, {
    ...
    headers: {
      ...
      "cache-control": response.headers.get("cache-control") || "public, max-age=0",
    },
  });
}

export const config = {
  matcher: ["/", "/documents/:path*", "/business/:path*", "/blogs/:path*", "/about"],
};
