...
function OpsToolsLogo({ size = 32 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {[0, 1, 2].map((row) =>
        [0, 1, 2].map((col) => (
          <rect key={`${row}-${col}`} x={6 + col * 7} y={6 + row * 7} width="5" height="5" rx="1.5" fill="white" opacity={0.3 + (row * 3 + col) * 0.08} />
        ))
      )}
    </svg>
  );
...
export default function Footer() {
  return (
    <footer style={{ background: "#07011F", borderTop: "1px solid rgba(255,255,255,0.07)", color: "#94A3B8", fontFamily: "inherit", position: "relative", overflow: "hidden" }}>
      ...
      <div className="footer-watermark"><OpsToolsWatermark /></div>
    </footer>
  );
}
