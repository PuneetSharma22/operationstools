      ...
      <div role="dialog" aria-modal="true" aria-label="Navigation menu" style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(320px, 85vw)", background: "#07011F", zIndex: 301, overflowY: "auto", display: "flex", flexDirection: "column", animation: "slideInRight 0.22s ease-out" }}>
        ...
        <div style={{ flex: 1, padding: "12px 0" }}>
          {sections.map(({ key, label, docs }) => (
            <div key={key}>
              <button onClick={() => setExpanded(expanded === key ? null : key)} aria-expanded={expanded === key} className="drawer-item" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", background: "transparent", border: "none", cursor: "pointer" }}>
                <span style={{ fontSize: 15, fontWeight: 600, color: "#fff" }}>{label}</span>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="2.5" style={{ transform: expanded === key ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}><path d="m6 9 6 6 6-6"/></svg>
              </button>
              {expanded === key && (
                <div style={{ padding: "4px 0 8px" }}>
                  {docs.map((doc) => {
                    ...
                    const inner = (
                      <div className="drawer-doc-item" style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 20px", opacity: doc.status === "soon" ? 0.5 : 1 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 9, flexShrink: 0, background: iconData?.bg || "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center" }}>{iconData?.svg}</div>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: 14, fontWeight: 600, color: isLive ? "#fff" : "rgba(255,255,255,0.55)" }}>{doc.name}</span>
                    ...
                    return isLive ? <Link key={doc.slug} to={doc.href} onClick={onClose} style={{ textDecoration: "none", display: "block" }}>{inner}</Link> : <div key={doc.slug}>{inner}</div>;
                  })}
                </div>
              )}
            </div>
          ...
          {user ? (
            <div style={{ padding: "8px 20px" }}>
              ...
              <Link to="/account" onClick={onClose} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 0", fontSize: 14, color: "rgba(255,255,255,0.75)", textDecoration: "none" }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>My Account
              </Link>
              <button onClick={async () => { await signOut(); onClose(); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 0", fontSize: 14, color: "#F87171", background: "none", border: "none", cursor: "pointer", width: "100%" }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#F87171" strokeWidth="2.5" strokeLinecap="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Sign out
              </button>
            </div>
          ) : (
...
function MegaDropdown({ docs, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", top: 64, left: "50%", transform: "translateX(-50%)", minWidth: cols === 3 ? 700 : 480, background: "#fff", border: "1px solid #E2E8F0", borderRadius: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.12)", padding: "24px 20px", zIndex: 200 }}>
      <style>{`@keyframes megaIn{from{opacity:0;transform:translateX(-50%) translateY(-4px) scale(0.98);}to{opacity:1;transform:translateX(-50%) translateY(0) scale(1);}} .nav-doc-item:hover{background:#F8FAFC!important;}`}</style>
        ...
        {Object.entries(groups).map(([bundle, items]) => (
          <div key={bundle} style={{ marginBottom: 8 }}>
            ...
            {items.map((doc) => {
              ...
              const isLive = doc.status === "live" || doc.status === "new";
              const item = (
                <div className="nav-doc-item" style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 8px", borderRadius: 10, background: "transparent", transition: "background 0.12s", cursor: isLive ? "pointer" : "default", marginBottom: 2 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: isLive ? "#0F172A" : "#94A3B8", whiteSpace: "nowrap" }}>{doc.name}</span>
                      <StatusBadge status={doc.status} />
                    </div>
              ...
              return isLive ? <Link key={doc.slug} to={doc.href} onClick={onClose} style={{ textDecoration: "none", display: "block" }}>{item}</Link> : <div key={doc.slug}>{item}</div>;
            })}
          </div>
        ))}
      </div>
      ...
      <header style={{ background: "#07011F", borderBottom: "1px solid rgba(255,255,255,0.08)", position: "sticky", top: 0, zIndex: 50 }} className="no-print">
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 20px", height: 64, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <nav aria-label="Main navigation" className="desktop-nav" style={{ display: "flex", alignItems: "center", gap: 4, flex: 1 }}>
            {NAV_ITEMS.map((item) => (
              <div key={item.key} style={{ position: "relative" }}>
                <button onClick={() => setOpenMenu(openMenu === item.key ? null : item.key)} aria-expanded={openMenu === item.key} aria-haspopup="true"
                ...
                >
                  {item.label}
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ transition: "transform 0.2s", transform: openMenu === item.key ? "rotate(180deg)" : "rotate(0deg)" }}><path d="m6 9 6 6 6-6"/></svg>
                </button>
              ...
              <>
                ...
                <div style={{ position: "relative" }}>
                  <button onClick={() => setShowUserMenu(!showUserMenu)} aria-label="User menu" aria-expanded={showUserMenu} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "0 12px", height: 36, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, cursor: "pointer" }}>
                    <div style={{ width: 20, height: 20, borderRadius: "50%", background: "linear-gradient(135deg,#2563EB,#4F46E5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "#fff" }}>{user.email[0].toUpperCase()}</div>
                  ...
                  {showUserMenu && (
                    <div style={{ position: "fixed", top: 64, right: 24, width: 210, background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12, boxShadow: "0 12px 40px rgba(0,0,0,0.12)", overflow: "hidden", zIndex: 200 }}>
                      ...
                      {[{ to: "/account", label: "My Account", icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#64748B" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg> }, { to: "/account", label: `${credits === null ? "—" : credits.toLocaleString()} credits`, icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#64748B" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg> }].map(({ to, label, icon }) => (
                        <Link key={label} to={to} onClick={closeAll} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", fontSize: 14, color: "#374151", textDecoration: "none" }} onMouseEnter={e => e.currentTarget.style.background="#F8FAFC"} onMouseLeave={e => e.currentTarget.style.background="transparent"}>{icon}{label}</Link>
                      ))}
                      <div style={{ borderTop: "1px solid #F1F5F9" }} />
                      <button onClick={async () => { await signOut(); closeAll(); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", fontSize: 14, color: "#DC2626", background: "none", border: "none", cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.background="#FEF2F2"} onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#DC2626" strokeWidth="2.5"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Sign out
                      </button>
                    </div>
                  )}
                </div>
          ...
          <div className="mobile-nav" style={{ display: "none", alignItems: "center", gap: 8, marginLeft: "auto" }}>
            {!user && <Link to="/signup" style={{ display: "inline-flex", alignItems: "center", height: 34, padding: "0 14px", fontSize: 13, fontWeight: 600, color: "#fff", textDecoration: "none", borderRadius: 8, background: "linear-gradient(135deg,#2563EB,#4F46E5)" }}>Sign up</Link>}
...
const FALLBACK_BUSINESS = [
  ...
  { slug: "tax-invoice", name: "Tax Invoice", href: "#", description: "Formal tax invoices for B2B", bundle: "GST Suite", status: "soon" },
];
