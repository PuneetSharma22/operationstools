...
export default function FreelancerInvoicePage() {
  ...
  const handlePDF = async () => {
    ...
    setDownloading(true);
    try {
      try { await supabase.from("save_requests").insert({ template:"freelancer-invoice", print_id:`FREEL-${Date.now()}`, user_id:null }); } catch(_){}
  ...
  return (
    ...
    <div style={{ backgroundColor:"#F8FAFC", minHeight:"100vh" }}>
      <style>{`@media(max-width:768px){.fl-grid{grid-template-columns:1fr!important;}.fl-prev{display:none!important;}}@media print{.no-print{display:none!important;}}`}</style>
