...
const defaultData = {
  ...
  fuelType: "Petrol", density: "625.078", quantity: "", pricePerLitre: "104.29",
      ...
      <div style={{ position: "relative", marginTop: "auto", background: "#fff", borderRadius: "20px 20px 0 0", maxHeight: "90vh", display: "flex", flexDirection: "column", animation: "sheetUp 0.25s cubic-bezier(0.16,1,0.3,1)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid #F1F5F9", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <button onClick={onDownload} disabled={downloading} style={{ background: "linear-gradient(135deg,#2563EB,#4F46E5)", border: "none", borderRadius: 8, padding: "8px 16px", color: "#fff", fontSize: 13, fontWeight: 600, cursor: downloading ? "wait" : "pointer", opacity: downloading ? 0.7 : 1, display: "flex", alignItems: "center", gap: 6 }}>
...
export default function FuelBillPage() {
  ...
  const doDownloadPDF = async (ref) => {
    ...
    } catch (err) { alert("PDF failed: " + (err?.message || "Unknown error")); }
    finally { setDownloading(false); }
  };

  const handleDownloadPDF = () => doDownloadPDF(showMobilePreview ? mobilePreviewRef : previewRef);

  const handleBulkClick = async () => {
    try {
      const { data: { user }, error } = await supabaseFull.auth.getUser();
      console.log("bulk click - user:", user, "error:", error);
      if (!user) { console.log("setting modal to login"); setModal("login"); }
      else { setModalUser(user); setModal("bulk"); }
    } catch(err) { 
      console.log("bulk click - catch:", err);
      setModal("login"); 
    }
  };

  useEffect(() => { console.log("modal state changed:", modal); }, [modal]);
  ...
  return (
    <div style={{ backgroundColor: "#F8FAFC", minHeight: "100vh" }}>
      <style>{`@media(max-width:768px){.desktop-preview{display:none!important;}.mobile-preview-btn{display:flex!important;}.fuel-tool-padding{padding-left:16px!important;padding-right:16px!important;}.fuel-hero-padding{padding:28px 16px 24px!important;}.seo-section{max-width:100%!important;width:100%!important;padding:0 16px!important;}}@media(min-width:769px){.mobile-preview-btn{display:none!important;}}`}</style>
      ...
      {showMobilePreview && <MobilePreviewSheet previewRef={mobilePreviewRef} PreviewComponent={PreviewComponent} data={data} onDownload={handleDownloadPDF} downloading={downloading} onClose={() => setShowMobilePreview(false)} />}
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "32px 24px" }} className="fuel-tool-padding no-print">
        <div className="no-print" style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10 }} className="template-grid">
            ...
            {TEMPLATES.map((t) => (
              <button key={t.id} onClick={() => setActiveTemplate(t.id)} style={{ textAlign: "left", padding: "14px", borderRadius: 12, border: activeTemplate === t.id ? "2px solid #2563EB" : "2px solid #E2E8F0", background: activeTemplate === t.id ? "#EFF6FF" : "#fff", cursor: "pointer", transition: "all 0.15s" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 28 }} className="tool-grid">
          ...
          <div className="no-print">
            ...
            <div className="mobile-preview-btn" style={{ marginTop: 20, gap: 10 }}>
              ...
              <button onClick={() => setShowMobilePreview(true)} style={{ flex: 1, height: 48, borderRadius: 12, border: "none", background: "linear-gradient(135deg,#2563EB,#4F46E5)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Save PDF
              </button>
            </div>
          </div>
          <div className="desktop-preview" style={{ position: "sticky", top: 96 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }} className="no-print">
              ...
              <button onClick={() => doDownloadPDF(previewRef)} disabled={downloading} className="no-print" style={{ background: "linear-gradient(135deg,#2563EB,#4F46E5)", border: "none", borderRadius: 8, padding: "6px 14px", color: "#fff", fontSize: 12, fontWeight: 600, cursor: downloading ? "wait" : "pointer", display: "flex", alignItems: "center", gap: 5, opacity: downloading ? 0.7 : 1 }}>
