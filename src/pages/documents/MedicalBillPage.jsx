...
export default function MedicalBillPage() {
  ...
  const handlePDF = async () => {
    ...
    setDownloading(true);
    try {
      try { await supabase.from("save_requests").insert({ template:"medical-bill", print_id:`MED-${Date.now()}`, user_id:null }); } catch(_){}
  ...
  return (
    <div style={{ backgroundColor:"#F8FAFC", minHeight:"100vh" }}>
      <style>{`@media(max-width:768px){.mb-grid{grid-template-columns:1fr!important;}.mb-prev{display:none!important;}}@media print{.no-print{display:none!important;}}`}</style>
