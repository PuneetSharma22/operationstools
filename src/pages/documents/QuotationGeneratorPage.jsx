...
export default function QuotationGeneratorPage() {
  ...
  const handlePDF = async () => {
    ...
    setDownloading(true);
    try {
      try { await supabase.from("save_requests").insert({ template:"quotation-generator", print_id:`QUOTE-${Date.now()}`, user_id:null, bill_data: { ...data, from, to, items } }); } catch (err) { console.warn("Save logging for the quotation failed (non-blocking); the document itself was unaffected.", err); }
