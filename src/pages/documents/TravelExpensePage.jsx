...
export default function TravelExpensePage() {
  ...
  const handlePDF = async () => {
    ...
    setDownloading(true);
    try {
      try { await supabase.from("save_requests").insert({ template:"travel-expense", print_id:`TRVL-${Date.now()}`, user_id:null }); } catch(_){}
  ...
  return (
    ...
    <div style={{ backgroundColor:"#F8FAFC", minHeight:"100vh" }}>
      <style>{`@media(max-width:768px){.te-grid{grid-template-columns:1fr!important;}.te-prev{display:none!important;}}@media print{.no-print{display:none!important;}}`}</style>
